...
class NetworkAnomalyDetector:
    ...
    def extract_features(self, packets_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features['day_of_week'] = packets_df['timestamp'].dt.dayofweek
            
            # Traffic volume features (per minute windows)
            packets_df['minute'] = packets_df['timestamp'].dt.floor('min')
            minute_stats = packets_df.groupby('minute').agg({
            ...
            packets_df = packets_df.merge(minute_stats, on='minute', how='left')
            
            # Add aggregated features
            features['packets_per_minute'] = packets_df['length_count']
            features['bytes_per_minute'] = packets_df['length_sum']
    ...
    def train_models(self, features_df: pd.DataFrame, save_path: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            pca = PCA(n_components=self.config['pca']['n_components'], 
            ...
            self.models['thresholds'] = {
                ...
                'byte_rate_95th': np.percentile(features_df['bytes_per_minute'], 95),
    ...
    def detect_anomalies(self, features_df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            stat_anomalies = np.zeros(len(features_df), dtype=bool)
            thresholds = self.models['thresholds']
            
            stat_anomalies |= (features_df['packets_per_minute'] > thresholds['packet_rate_95th'])
            stat_anomalies |= (features_df['bytes_per_minute'] > thresholds['byte_rate_95th'])
            stat_anomalies |= (features_df['port_diversity'] > thresholds['port_diversity_95th'])
            stat_anomalies |= (features_df['ip_diversity'] > thresholds['ip_diversity_95th'])
            stat_anomalies |= (features_df['potential_port_scan'] == 1)
            
            anomalies['statistical'] = stat_anomalies.astype(int)
            ...
            stat_anomalies['combined'] = (combined_score >= 2).astype(int)
            
            # Calculate anomaly scores
            anomaly_scores = {}
            if 'isolation_forest' in self.models:
                anomaly_scores['isolation_forest'] = self.models['isolation_forest'].decision_function(features_scaled)
...
def main():
    ...
    sample_packets.loc[970:980, 'dst_port'] = list(range(1000, 1011))  # Port scan (11 values for 11 rows)
